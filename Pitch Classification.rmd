---
title: "Pitch Classification by Cluster Analysis"
author: "Billy Lozowski"
date: "`r Sys.Date()`"
output:
  html_document:   # specify output document type
    toc: true      # include a table of contents
  pdf_document:
  md_document:     # render as a markdown file
    variant: gfm   # specify that you want to save the output as a markdown file for github
---
...
```{r install and load required packages, include=FALSE}

# list packages needed for this analysis
packages <- c("mclust", "umap", "plotly", "scales", "tidyverse", "purrr", "dbscan", "proxy", "pheatmap", "RColorBrewer")

# install only the ones that aren't already installed
...
## "mclust" - GMM analysis
## "umap" - visualising cluster models
## "plotly" - interactive model plots
## "scales" - scale variables
## "tidyverse" - 
## "purrr" - for boxplots
## "dbscan" - DBSCAN analysis
## "proxy" - similarity matrices
## "pheatmap" - heatmap for similarity matrix
## "RColorBrewer" - colour scale for heatmaps
...
```{r rename columns of interest, include=FALSE}
...
```{r metric conversion, include=FALSE}
...
```{r right handed pitchers, echo=FALSE}

rX <- scale(d_right[, features2]) # scale features (break profile only)

# fit Gaussian Mixture Model (GMM)
gmm_modelr <- Mclust(rX)
best_kr <- gmm_modelr$G
d_right$cluster <- as.factor(gmm_modelr$classification)
cat("Optimal number of clusters for RHP based on break profile (GMM):", best_kr, "\n")
...
r_umap_df <- data.frame(
  ...
  vertical_release_angle = d_right$vertical_release_angle,
  horizontal_release_angle = d_right$horizontal_release_angle,
  spin_rate = d_right$spin_rate,
  spin_axis = d_right$spin_axis,
  release_height = d_right$release_height,
  release_side = d_right$release_side,
  extension = d_right$extension,
  VB = d_right$vb,
...
plot_rhp <- plot_ly(
  ...
  text = ~paste(
    "Cluster:", cluster,
    "<br>Manual:", TaggedPitchType,
    "<br>Auto:", AutoPitchType,
    
    "<br>Velocity:", paste0(round(velocity, 1), "m/s"),
    "<br>Vertical Angle:", paste0(round(vertical_release_angle, 1), "deg"),
    "<br>Horizontal Angle:", paste0(round(horizontal_release_angle, 1), "deg"),
    "<br>Spin Rate:", paste0(round(spin_rate, 0), "rpm"),
    "<br>Spin Axis:", paste0(round(spin_axis, 1), "deg"),
    "<br>Release Height:", paste0(round(release_height, 2), "m"),
    "<br>Release Side:", paste0(round(release_side, 2), "m"),
    "<br>Release Extension:", paste0(round(extension, 2), "m"),
    
    "<br>Induced VB:", paste0(round(inducedVB, 2), "m"),
    "<br>VB:", paste0(round(VB, 2), "m"),
    "<br>HB:", paste0(round(HB, 2), "m")
  ),
...
```{r left handed pitchers, echo=FALSE}
...
cat("Optimal number of clusters for LHP based on break profile (GMM):", best_kl, "\n")
...
l_umap_df <- data.frame(
  ...
  vertical_release_angle = d_left$vertical_release_angle,
  horizontal_release_angle = d_left$horizontal_release_angle,
  spin_rate = d_left$spin_rate,
  spin_axis = d_left$spin_axis,
  release_height = d_left$release_height,
  release_side = d_left$release_side,
  extension = d_left$extension,
  VB = d_left$vb,
...
View cluster plots
```{r UMAP plots, echo=FALSE, warning=FALSE, message=FALSE}

# display the interactive plots below
print(plot_rhp)
print(plot_lhp)

htmlwidgets::saveWidget(
  widget = plot_rhp,
  file = "Figures/UMAP for RHP Break Profile (GMM).html",
  selfcontained = TRUE
)

htmlwidgets::saveWidget(
  widget = plot_lhp,
  file = "Figures/UMAP for LHP Break Profile (GMM).html",
  selfcontained = TRUE
)

# embed the html widgets in the knitted/rendered file
htmltools::includeHTML("Figures/UMAP for RHP Break Profile (GMM).html")
htmltools::includeHTML("Figures/UMAP for LHP Break Profile (GMM).html")
...
```{r UMAP with facet wrapping, echo=FALSE, warning=FALSE, message=FALSE}
...
ggsave(UMAP_rhp, file = "Figures/UMAP Faceted by Cluster - RHP (GMM).png", dpi = 600)
ggsave(UMAP_lhp, file = "Figures/UMAP Faceted by Cluster - LHP (GMM).png", dpi = 600)
...
Summary of ball metrics by cluster (IQRs)
```{r IQR summary, echo=FALSE}
...
write.csv(cluster_summary_rhp, file = "Data/RHP Ball Metric Summary (by cluster - GMM).csv", row.names = FALSE)
...
write.csv(cluster_summary_lhp, file = "Data/LHP Ball Metric Summary (by cluster - GMM).csv", row.names = FALSE)

print(cluster_summary_rhp)
print(cluster_summary_lhp)

```

Summary of pitch frequency by cluster (auto- and manually-tagged)
```{r pitches included in each cluster, echo=FALSE}
...
write.csv(pitchtype_summary_rhp_auto, file = "Data/RHP Pitch Frequency by Cluster (auto-tagged - GMM).csv", row.names = FALSE)
...
write.csv(pitchtype_summary_rhp_tagged, file = "Data/RHP Pitch Frequency by Cluster (manually tagged - GMM).csv", row.names = FALSE)
...
# save the auto tagged cluster summary data for RHP
write.csv(pitchtype_summary_lhp_auto, file = "Data/LHP Pitch Frequency by Cluster (auto-tagged - GMM).csv", row.names = FALSE)
...
# save the tagged cluster summary data for RHP
write.csv(pitchtype_summary_lhp_tagged, file = "Data/LHP Pitch Frequency by Cluster (manually tagged - GMM).csv", row.names = FALSE)

print(pitchtype_summary_rhp_auto)
print(pitchtype_summary_lhp_auto)
print(pitchtype_summary_rhp_tagged)
print(pitchtype_summary_lhp_tagged)

```

Visual summary of each cluster's ball metrics
```{r cluster box plots + facet_wrap(), include=FALSE}
...
```{r cluster box plots with purrr, echo=FALSE, warning=FALSE, message=FALSE}
...
walk2(.x = plots_r,
      .y = names(plots_r),
      .f = ~ ggsave(filename = paste0("Figures/RHP - ", .y, ".png"),
                    plot = .x,
...
walk2(.x = plots_l,
      .y = names(plots_l),
      .f = ~ ggsave(filename = paste0("Figures/LHP - ", .y, ".png"),
                    plot = .x,
                    width = 7, height = 5, dpi = 600)
      )
...
```{r GGally - line plot, eval=FALSE, include=FALSE}
...
cat("Optimal number of clusters for RHP based on break profile (DBSCAN):", length(unique(hdb_r$cluster[hdb_r$cluster > 0])), "\n")
...
cat("Optimal number of clusters for LHP based on break profile (DBSCAN):", length(unique(hdb_l$cluster[hdb_l$cluster > 0])), "\n")

```


```{r visualise DBSCAN models with UMAP, echo=FALSE, warning=FALSE, message=FALSE}
...
htmlwidgets::saveWidget(
  widget = plot_hdb_rhp,
  file = "Figures/UMAP for RHP Break Profile (DBSCAN).html",
  selfcontained = TRUE
)

htmlwidgets::saveWidget(
  widget = plot_hdb_lhp,
  file = "Figures/UMAP for LHP Break Profile (DBSCAN).html",
  selfcontained = TRUE
)

# embed the interactive plots in the knitted/rendered file
htmltools::includeHTML("Figures/UMAP for RHP Break Profile (DBSCAN).html")
htmltools::includeHTML("Figures/UMAP for LHP Break Profile (DBSCAN).html")

```


```{r summary data - DBSCAN, echo=FALSE}
...
# save the summary data for RHP
write.csv(summary_stats_rhp, file = "Data/RHP Ball Metric Summary (by cluster - DBSCAN).csv", row.names = FALSE)

# save the tagged cluster summary data for RHP
write.csv(freq_summary_rhp_tagged, file = "Data/RHP Pitch Frequency by Cluster (manually tagged - DBSCAN).csv", row.names = FALSE)

# save the auto cluster summary data for RHP
write.csv(freq_summary_rhp_auto, file = "Data/RHP Pitch Frequency by Cluster (auto tagged - DBSCAN).csv", row.names = FALSE)
...
# save the summary data for LHP
write.csv(summary_stats_lhp, file = "Data/LHP Ball Metric Summary (by cluster - DBSCAN).csv", row.names = FALSE)

# save the tagged cluster summary data for LHP
write.csv(freq_summary_lhp_tagged, file = "Data/LHP Pitch Frequency by Cluster (manually tagged - DBSCAN).csv", row.names = FALSE)

# save the auto cluster summary data for LHP
write.csv(freq_summary_lhp_auto, file = "Data/LHP Pitch Frequency by Cluster (auto tagged - DBSCAN).csv", row.names = FALSE)

```


Some of the clusters identified in both the GMM and HDBSCAN analyses seem to overlap, or don't necessarily look that different from each other. We are, therefore, going to create a quick similarity matrix to assess which clusters look most similar to each other (using Euclidean distance).


$$
\text{Distance} = \sqrt{(x_1 - x_2)^2 + (y_1 - y_2)^2 + \dots + (n_1 - n_2)^2}
$$


```{r similarity matrix, echo=FALSE}

RHP_summary <- read.csv("Data/RHP Ball Metric Summary (by cluster - GMM).csv")
LHP_summary <- read.csv("Data/LHP Ball Metric Summary (by cluster - GMM).csv")

# select only mean columns
RHP_mean_cols <- names(RHP_summary)[grepl("_mean$", names(RHP_summary))]
LHP_mean_cols <- names(LHP_summary)[grepl("_mean$", names(LHP_summary))]

# extract mean data per cluster
RHP_cluster_means <- RHP_summary %>%
  select(cluster, all_of(RHP_mean_cols)) %>%
  column_to_rownames("cluster")  # use cluster number as row names

LHP_cluster_means <- LHP_summary %>%
  select(cluster, all_of(LHP_mean_cols)) %>%
  column_to_rownames("cluster")

# calculate pairwise Euclidean distances
RHP_similarity_matrix <- as.matrix(round(dist(RHP_cluster_means, method = "euclidean")), 0)
LHP_similarity_matrix <- as.matrix(round(dist(LHP_cluster_means, method = "euclidean")), 0)

# matrix heatmaps
png("Figures/RHP Cluster Similarity Matrix (Euclidean Distance).png", width = 6, height = 6, units = "in", res = 600)

pheatmap(RHP_similarity_matrix,
         display_numbers = TRUE,
         number_format = "%.0f",
         main = "RHP Cluster Similarity Matrix (Euclidean Distance)",
         fontsize_number = 10,
         color = colorRampPalette(brewer.pal(9, "YlGnBu"))(100))

dev.off()

png("Figures/LHP Cluster Similarity Matrix (Euclidean Distance).png", width = 6, height = 6, units = "in", res = 600)

pheatmap(LHP_similarity_matrix,
         display_numbers = TRUE,
         number_format = "%.0f",
         main = "LHP Cluster Similarity Matrix (Euclidean Distance)",
         fontsize_number = 10,
         color = colorRampPalette(brewer.pal(9, "YlGnBu"))(100))

dev.off()

# knitr::include_graphics("Figures/RHP Cluster Similarity Matrix (Euclidean Distance).png")
# knitr::include_graphics("Figures/LHP Cluster Similarity Matrix (Euclidean Distance).png")

write.csv(RHP_similarity_matrix, "Data/RHP Cluster Similarity Matrix.csv")
write.csv(LHP_similarity_matrix, "Data/LHP Cluster Similarity Matrix.csv")
```

```{r eval=FALSE}

print(RHP_similarity_matrix)
print(LHP_similarity_matrix)
