...
install.packages(c("mclust", "umap", "plotly", "scales", "tidyverse", "dbscan", "proxy", "pheatmap", "RColorBrewer"))
```

First, let's load the packages we need to perform this analysis
```{r load packages, message=FALSE}
library(mclust)       # GMM analysis
library(umap)         # visualising cluster models
library(plotly)       # interactive model plots
library(scales)       # scale variables
library(tidyverse)
library(dbscan)       # DBSCAN analysis
library(proxy)        # similarity matrices
library(pheatmap)     # heatmap for similarity matrix
library(RColorBrewer) # colour scale for heatmaps
```
...
htmlwidgets::saveWidget(
  widget = plot_rhp,
  file = "UMAP for RHP Break Profile (GMM).html",
  selfcontained = TRUE
)

htmlwidgets::saveWidget(
  widget = plot_lhp,
  file = "UMAP for LHP Break Profile (GMM).html",
  selfcontained = TRUE
)
...
ggsave(UMAP_rhp, file = "UMAP Faceted by Cluster - RHP (GMM).png", dpi = 600)
ggsave(UMAP_lhp, file = "UMAP Faceted by Cluster - LHP (GMM).png", dpi = 600)
...
write.csv(RHP_similarity_matrix, "RHP Cluster Similarity Matrix.csv")
write.csv(LHP_similarity_matrix, "LHP Cluster Similarity Matrix.csv")
